        ...
        if(request.isNull("database") && request.isNull("collection") || request.isNull("database") && !request.isNull("collection")){
            ...
            try {
                if(request.isNull("collection")){
                    ...
                    RoseServer.reply(context, "Successfully deleted the collection " + request.getString("collection"), 1);
                }
            } catch (IOException exception) {
                RoseServer.reply(context, "An exception occurred: " + exception.getMessage(), -1);
            }
        }
    }
}
